...
echo "Build Docker Prometheus image on `date`"
...
docker build --build-arg "BUILD_IMAGE=${DOCKER_IMAGE_HASH}" -t prometheus .

docker tag prometheus:latest ${AWS_ACCOUNT_ID}.dkr.ecr.eu-west-2.amazonaws.com/prometheus:${CODEBUILD_RESOLVED_SOURCE_VERSION}-${CODEBUILD_START_TIME}

echo "Push Docker image on `date`"
docker push ${AWS_ACCOUNT_ID}.dkr.ecr.eu-west-2.amazonaws.com/prometheus:${CODEBUILD_RESOLVED_SOURCE_VERSION}-${CODEBUILD_START_TIME}
